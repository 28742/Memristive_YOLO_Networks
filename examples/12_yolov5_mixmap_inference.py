...
def main():
    ...
    parser.add_argument("--mixmap", action="store_true", help="启用 MixMap 混合层架构计算密度自动分配映射")
    parser.add_argument("--tau-thresh", type=float, default=0.02, help="MixMap的小负数判别阈值tau")
    parser.add_argument("--density-thresh", type=float, default=0.1, help="MixMap层指派阈值(比值大于此数值启用双阵列)")
    parser.add_argument("--zero-thresh", type=float, default=0.01, help="单阵列层的微小负数置零保护阈值")
    ...
    if args.mem_enabled:
        if args.mixmap:
            from hybrid_mapping_analyzer import analyze_and_apply_hybrid_mapping
            with torch.no_grad():
                yolo = analyze_and_apply_hybrid_mapping(
                    model=yolo,
                    tau_thresh=args.tau_thresh,
                    density_threshold=args.density_thresh,
                    zero_thresh_fallback=args.zero_thresh
                )
            print("[MemIntelli][MixMap] 应用混合架构 (MixMap): 已根据网络分布自动分配双阵列/单阵列。")
